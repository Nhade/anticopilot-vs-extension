...
- \`src/commands/\`: All command palette actions (e.g., \`reportStruggle.ts\`). Each file should generally export a single \`registerXCommand()\` function returning a \`vscode.Disposable\`.
...
- Command-driven triggers (like \`anticopilot.reportStruggle\`) should only scrape the bare minimum context needed (e.g., ±10 lines of code around the active selection).
...
- Commands (like \`reportStruggle\`) must query the \`SidebarProvider\` to attach the correct IDs to their API payloads.
